...
export const parseDateToYMD = (date) => {
  let tempDate = date;
  if (tempDate === null || tempDate === undefined) return '';
  if (typeof tempDate === 'string') {
    tempDate = new Date(tempDate);
  }
  const year = tempDate.getFullYear();
...
/**
 * "2505121949212" → "250512194921-2"
 */
export const parseOrderNum = (value) => {
  if (value.length < 13) return value; // 길이 부족하면 그대로 반환

  const timestamp = value.slice(0, 12); // 앞 12자리
  const id = value.slice(12); // 나머지
...
export const parseOnlyNum = (value) => {
  if (isEmpty(value)) {
    return '0';
  }
