...
const FindPage = () => {
  ...
  const handleFind = useCallback(async () => {
    if (tabIndex === 0) {
      if (!name) {
        return openModal({
          text: localeText(LANGUAGES.INFO_MSG.ENTER_BRAND_NAME),
        });
      }
      if (!pw) {
        return openModal({
      ...
      const result = await sellerUserApi.getSellerFindEmail(param);
      ...
      const result = await sellerUserApi.getSellerFindPassword(param);
      if (result?.errorCode === SUCCESS) {
        setResultData(result.data);
        setIsComplete(true);
      } else {
