...
import { copyToClipboard, isEmpty, removeNullValues } from './commonUtils';
...
export const parseDateToStr = (
  ...
  let tempDate = date;
  if (tempDate === null || tempDate === undefined) return undefined;
...
export const parseDateToYMD = (date) => {
  let tempDate = date;
  if (tempDate === null || tempDate === undefined) return null;
  if (typeof tempDate === 'string') {
    tempDate = new Date(tempDate);
  }
  const year = tempDate.getFullYear();
...
export const parseBusinessNum = (input) => {
  if (!input) return;
  // 숫자만 남기고, 나머지는 제거
  const cleanedInput = input.replace(/\D/g, '');
  // 숫자가 10자리일 때, 하이픈 추가하여 반환
  if (cleanedInput.length === 10) {
    return cleanedInput.replace(/(\d{3})(\d{2})(\d{5})/, '$1-$2-$3');
  }
  // 10자리가 아닌 경우 그대로 반환
  return input;
};

export const parseSortList = (datas, key) => {
  datas.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
  return datas;
};
...
export const parseDateByCountryCode = (
  ...
  if (dateString.length === 6 || dateString.length === 8) {
    date = parseDateFromSimpleStr(dateString);
  } else {
